...
fn parse_ipv4_hex(hex: &str) -> String {
    if hex.len() != 8 { return "0.0.0.0".to_string(); }
...
fn parse_ipv6_hex(hex: &str) -> String {
    ...
    let parts: Vec<String> = (0..8)
        ...
        .filter_map(|i| {
            let idx = i * 4;
            if idx + 4 <= hex.len() {
                Some(format!("{:x}", u16::from_str_radix(&hex[idx..idx+4], 16).ok()?))
            } else {
                None
            }
        })
                        ...
                        if let Ok(port) = port_str.parse::<u16>() {
                            if let Some(pid_str) = parts.get(4) {
                                if let Ok(pid) = pid_str.parse::<u64>() {
                                    sockets.insert(pid, SocketInfo {
                                        port,
                                        state,
                                        local_addr: addr.trim_start_matches('[').trim_end_matches(']').to_string(),
                                        remote_addr: String::new(),
                                        explanation: String::new(),
                                        workaround: String::new(),
                                    });
...
pub fn get_listening_sockets() -> HashMap<u64, SocketInfo> {
    ...
    if let Ok(output) = Command::new("lsof").args(["-iTCP", "-sTCP:LISTEN", "-nP"]).output() {
        let output_str = String::from_utf8_lossy(&output.stdout);
