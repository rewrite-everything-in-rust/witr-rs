    ...
    if let Ok(output) = Command::new("netstat").args(["-ano"]).output() {
        ...
        for line in stdout.lines() {
            ...
            if proto == "TCP" {
                ...
                // UDP doesn't have state column in same way, but it's effectively listening
...
pub fn get_sockets_for_pid(target_pid: u32) -> Vec<u64> {
    ...
    if let Ok(output) = Command::new("netstat").args(["-ano"]).output() {
        ...
        for line in stdout.lines() {
            ...
            let proto = parts[0];
            let local_addr = parts[1];
            let pid_str = parts.last().unwrap(); // PID is reliably last for both TCP/UDP lines we care about
